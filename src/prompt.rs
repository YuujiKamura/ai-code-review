...
/// QDD (Question-Driven Development) prompt - generates questions, not answers
pub const QDD_PROMPT: &str = r#"以下のコード変更に対して、「答え」ではなく「問い」を生成してください。
良い問いは思考の分岐点を可視化し、設計判断の見落としを防ぎます。
...
1. **設計意図** - なぜこの設計を選んだのか。別の選択肢はなかったか
2. **エッジケース** - 想定外の入力・状態で何が起きるか
3. **将来の保守性** - 半年後の自分（または他の開発者）が理解できるか
4. **パフォーマンス** - スケールしたとき問題にならないか
5. **テストカバレッジ** - この変更を壊すテストが存在するか

## 出力形式

各カテゴリについて、関連する問いがあれば生成。なければスキップ。
...
- 「答え」や「修正案」は書くな。問いだけを書け
- 自明な問い（「テストはあるか？」等）は書くな。コードを読んで初めて気づく問いだけ書け
- 最大10個まで。質より量を優先するな
"#;
...
pub enum PromptType {
    ...
    Multi,
    /// QDD (Question-Driven Development) - generates questions instead of answers
    Qdd,
